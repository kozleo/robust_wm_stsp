...
from stabda.utils import get_session_ids, xr_save, get_results_path, get_distr_on_times


def run():
    """Run experiment for decoding pre-distractor activity vs post-distractor
    ...
    save_name = f"decode_baseline_distr_{args.kernel_width}_{args.bin_size}.nc"
...
def run_session_decode(session, kernel_width, bin_size):
    """
    Run decoding pre-distractor activity vs post-distractor activity for
    individual session.
    ...
    sample_on = 2000
    baseline_pre_distr = 350
    distr_on_times = get_distr_on_times()
    ...
    for delay in delays:
        print(delay)
        # set delay specific distr on times
        distr_start_sec = distr_on_times[delay]

        distr_on = sample_on + int(distr_start_sec * 1000)
        distr_l = distr_on - bin_size

        baseline_start = distr_l - baseline_pre_distr
        baseline_end = distr_on - baseline_pre_distr
        ...
        for has_distr in [0, 1]:
            ...
            for s, samp in enumerate(samples):
                ...
                # select pre-distractor baseline period (delay specific)
